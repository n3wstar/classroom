...
async function loadEditMedia(roomId) {
    ...
    try {
        ...
        media.forEach(m => {
            ...
            div.innerHTML = `
                <img src="${m.url}" style="width: 100%; height: 100%; object-fit: cover;" onerror="this.src='https://via.placeholder.com/100?text=No+image'">
...
async function uploadEditMedia() {
    ...
    const endpoint = mediaType === 'photo' 
        ? `http://localhost:3000/api/upload/photo/${roomId}`
        : `http://localhost:3000/api/upload/panorama/${roomId}`;
